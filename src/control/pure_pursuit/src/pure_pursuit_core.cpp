...
PurePursuitNode::PurePursuitNode()
  ...
  , direction_(WaypointArrayDirection::Forward)
  , velocity_source_(-1)
...
void PurePursuitNode::initForROS()
{
  ...
  // setup subscriber
  sub1_ = nh_.subscribe("/casper_auto/final_waypoints", 10, &PurePursuitNode::callbackFromWayPoints, this);
  sub2_ = nh_.subscribe("casper_auto/current_pose", 10, &PurePursuitNode::callbackFromCurrentPose, this);
  sub3_ = nh_.subscribe("/casper_auto/config/waypoint_follower", 10, &PurePursuitNode::callbackFromConfig, this);
  sub4_ = nh_.subscribe("/casper_auto/current_velocity", 10, &PurePursuitNode::callbackFromCurrentVelocity, this);
...
{
  int sgn = 0;
  if (direction_ == WaypointArrayDirection::Forward)
  {
    sgn = 1;
  }
  else if (direction_ == WaypointArrayDirection::Backward)
  {
    sgn = -1;
  }
  return sgn;
...
{
  ROS_INFO("Received current pose in pure pursuit.");
...
{
  ROS_INFO("Received current velocity in pure pursuit.");
  current_linear_velocity_ = msg->twist.linear.x;
  pp_.setCurrentVelocity(current_linear_velocity_);
  is_velocity_set_ = true;
}

void PurePursuitNode::callbackFromWayPoints(const casper_auto_msgs::WaypointArrayConstPtr& msg)
{
  ROS_INFO("Received final wapoints in pure pursuit.");
  command_linear_velocity_ = (!msg->waypoints.empty()) ? msg->waypoints.at(0).twist.twist.linear.x : 0;
  if (add_virtual_end_waypoints_)
  {
    const WaypointArrayDirection solved_dir = getWaypointArrayDirection(*msg);
    direction_ = (solved_dir != WaypointArrayDirection::Error) ? solved_dir : direction_;
    casper_auto_msgs::WaypointArray expanded_waypoint_array(*msg);
    expand_size_ = -expanded_waypoint_array.waypoints.size();
    connectVirtualLastWaypoints(&expanded_waypoint_array, direction_);
    expand_size_ += expanded_waypoint_array.waypoints.size();

    pp_.setCurrentWaypoints(expanded_waypoint_array.waypoints);
  }
  else
  {
...
void PurePursuitNode::connectVirtualLastWaypoints(casper_auto_msgs::WaypointArray* waypoint_array, WaypointArrayDirection direction)
{
  if (waypoint_array->waypoints.empty())
  {
    return;
  }
  static double interval = 1.0;
  const geometry_msgs::Pose& pn = waypoint_array->waypoints.back().pose.pose;
  casper_auto_msgs::Waypoint virtual_last_waypoint;
  ...
  for (double dist = minimum_lookahead_distance_; dist > 0.0; dist -= interval)
  {
    ...
    waypoint_array->waypoints.emplace_back(virtual_last_waypoint);
  }
}
