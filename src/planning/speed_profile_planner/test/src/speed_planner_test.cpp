/**
 * @file speed_planner_test.h
 * @brief SpeedPlanner class test
 **/
...
public:
  SpeedPlannerTest() = default;

  SpeedPlannerTest(STGraph st_graph) { Run(); }
...
std::vector<Point2D> SpeedPlannerTest::LoadReferencePath(std::string filename) {
...
void SpeedPlannerTest::PrintResult(
...
void SpeedPlannerTest::SaveResult(
    std::vector<std::vector<STCell>> candidate_plans) {
...
int SpeedPlannerTest::Run() {
  DEBUG_MSG("");
  DEBUG_MSG("SpeedPlannerTest class Test");
  ...
  SpeedPlanner sp;
  // sp.SetMapFolder("../data");
  // sp.InitScenario(ScenarioID::ENTER_ROAD);

  std::vector<TrajectoryPoint> reference_trajectory;

  for (auto e : reference_path) {
    TrajectoryPoint p;
    p.x = e.x;
    p.y = e.y;
    p.v_limit = 10.0;
  ...
  reference_trajectory[0].v = 0.0;

  sp.Plan(obstacles, reference_trajectory);

  DEBUG_MSG("Result");
  for (auto p : reference_trajectory) {
    std::cout << "(" << p.x << ", " << p.y << ", " << p.v << ") " << std::endl;
  }
...
int main(int argc, char *argv[]) {
  /** test: speed planner class **/
  DEBUG_MSG("***** SpeedPlanner Test: *****");

  SpeedPlannerTest sp_test;
  sp_test.Run();
