...
class RegionOfInterest(object):
    ...
    def __init__(self, role_name, debug=False):
        ...
        merge_pt_src_param = rospy.get_param('~merging_point')
        if merge_pt_src_param:
            merge_pt_src = merge_pt_src_param.split(',')
            if len(merge_pt_src) != 3:
                raise ValueError("Invalid spawnpoint '{}'".format(merge_pt_src_param))
            self.merge_pt_src = [float(merge_pt_src[0]), float(merge_pt_src[1]), float(merge_pt_src[2])]
    ...
    def objects_updated(self, object_array):
        ...
        for i in xrange(len(self.objects)):
            ...
            if obj_conv and abs(obj_d) <= 0.5 and obj_s < self.merge_pt_src_frenet_s:
                veh = VehicleState()
            ...
            if obj_conv and abs(obj_d) <= 0.5 and not (is_source_follower or is_source_leader):
                veh = VehicleState()
                veh.header = obj.header
                veh.lifetime_id = obj.id
                veh.pose.pose = obj.pose
    ...
    def source_lane_line_updated(self, path):
        ...
        obj_s, obj_d, obj_conv = get_frenet(self.merge_pt_src[0], self.merge_pt_src[1], self.source_lane_line_list, self.source_lane_s_map)

        if obj_conv:
            self.merge_pt_src_frenet_s = obj_s
            print("Merge point frenet: [%f, %f]" % (obj_s, obj_d))
